/*!
 * @file            GParserYaml_loadFloatArray.c
...
int GParserYaml_loadFloatArray(GParserYaml_State *p_state_in, const char *p_key_in, float *p_values_out, size_t maxCount_in, size_t *p_count_out)
{
    ...
    for (i = 0; i < p_sequence->numChildren && count < maxCount_in; i++)
    {
        ...
        {
            p_values_out[count++] = (float)p_sequence->p_children[i].numberValue;
        }
        else
        {
            return GCONST_FALSE;
