/*!
 * @file            GParserJson_loadIntArray.c
...
int GParserJson_loadIntArray(GParserJson_State *p_state_in, const char *p_key_in, int *p_values_out, size_t maxCount_in, size_t *p_count_out)
{
    ...
    for (i = 0; i < p_array->numChildren && count < maxCount_in; i++)
    {
        ...
        {
            p_values_out[count++] = (int)p_array->p_children[i].numberValue;
        }
        else
        {
            return GCONST_FALSE;
