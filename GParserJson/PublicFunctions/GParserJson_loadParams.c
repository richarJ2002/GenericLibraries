...
int GParserJson_loadParams(GParserJson_State *p_state_in, const char *p_filePath_in)
{
    FILE *p_file = NULL;
    long fileSize = 0;
    size_t bytesRead = 0;
    ...
    GParserJson_clearBuffers(p_state_in);
    ...
    if (fileSize <= 0 || fileSize > GPARSERJSON_MAX_FILE_SIZE)
    {
        fclose(p_file);
        return GCONST_FALSE;
    }
    ...
    p_state_in->state = GPARSERJSON_STATE_START;
    p_state_in->depth = 0;

    /* Parse JSON */
    status = GParserJson_parseValue(p_state_in, &p_state_in->p_root);
